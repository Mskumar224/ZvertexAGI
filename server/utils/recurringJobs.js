...
function scheduleRecurringJobs() {
  cron.schedule('*/30 * * * *', async () => {
    ...
    for (const user of users) {
      if (!user.selectedCompanies || !user.selectedTechnology) continue;
      ...
      const isFullApply = user.profileDetails && Object.keys(user.profileDetails).length > 0;
      ...
      const jobs = await Promise.all(companies.map(async (company, index) => {
        ...
        try {
          ...
          return {
            ...
            company: job.company,
            link: job.url,
            requiresDocs: isFullApply || Math.random() > 0.5,
          };
        } catch (error) {
          return {
            id: `${uniqueId}-1`,
      ...
      for (const job of jobs) {
        if (!appliedIds.includes(job.id) && submissionsLeft > 0) {
          const profile = isFullApply && profiles.length > 0 ? profiles[0] : null;
