...
router.post('/verify-companies', async (req, res) => {
  ...
  try {
    const results = await Promise.all(companies.map(async (company, index) => {
      if (index > 0) await delay(1000); // Rate limiting
      try {
        const response = await axios.get(`https://www.google.com/search?q=${encodeURIComponent(company + ' official website')}`, { timeout: 5000 });
...
router.post('/fetch-jobs', async (req, res) => {
  ...
  try {
    ...
    const jobs = companies.flatMap(company => [
      { id: `${company}-${Date.now()}-1`, title: `${technology} Engineer at ${company}`, company, link: `https://${company.toLowerCase().replace(/\s+/g, '')}.com/jobs/1`, requiresDocs: false },
      { id: `${company}-${Date.now()}-2`, title: `${technology} Analyst at ${company}`, company, link: `https://${company.toLowerCase().replace(/\s+/g, '')}.com/jobs/2`, requiresDocs: true },
    ]);
